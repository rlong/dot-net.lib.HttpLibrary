...
namespace jsonbroker.library.common.log
{
    public class TraceLogDelegate : LogDelegate 
    {
        private bool _isDebugEnabled;

        public TraceLogDelegate(bool isDebugEnabled)
        {
        ...
        public void info(Log origin, String message)
        {
            String line;
            {
                if (_isDebugEnabled)
                {
                    String threadName = System.Threading.Thread.CurrentThread.Name;
                    String methodName = LogDelegateHelper.getMethodName(origin);
                    line = String.Format("INF {0} [{1} {2}] {3}", threadName, origin.CallerClassName(), methodName, message);
                }
                else
                {
                    String threadName = System.Threading.Thread.CurrentThread.Name;
                    line = String.Format("INF {0} [{1} -] {2}", threadName, origin.CallerClassName(), message);
                }
            }
            Trace.WriteLine(line);
